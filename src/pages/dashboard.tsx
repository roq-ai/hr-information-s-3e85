...
import { getEmployeeData } from 'apiSdk/employee-data';
...
function DashboardPage() {
  ...
  const employeeDataStat = useSWR<PaginatedInterface<EmployeeDataInterface>>(
    () => `/employee-data`,
    () => getEmployeeData({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Employee Data',
      result: employeeDataStat?.data?.totalCount,
      isLoading: employeeDataStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Team Members',
